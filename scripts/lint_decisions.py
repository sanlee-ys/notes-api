...
only shrink. Both existing ADRs here predate the rule and are grandfathered rather than
backfilled — an author following this repo's own conventions at the time produced a
compliant document, so the fault was the missing instruction, not the documents. New
ADRs must comply.
...
# ADRs predating the ported rule (2026-07-19). THIS LIST MAY ONLY SHRINK — removing an
# entry means that ADR grew a real Downstream surfaces section; adding one would mean
# the rule was weakened to fit a document instead of the reverse.
LEGACY_NO_DOWNSTREAM: set[str] = {"001", "002"}
